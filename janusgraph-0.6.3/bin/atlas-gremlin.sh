...
cd $(dirname $0)
DIR=`pwd`
#ATLAS_WEBAPP_DIR='/Users/nikhil/pro/repos/atlas/webapp/'
ATLAS_WEBAPP_DIR='/opt/apache-atlas/server/webapp/atlas/'
JAAS_CONF_FILE=
#=infra_solr_jaas.conf
...
CP="$CP":$(find -L ${DIR}/../conf -name '*.xml' | sort | tr '\n' ':')
CP="$CP":$(find -L $LIB -name 'slf4j-log4j12*.jar' | sort | tr '\n' ':')
#CP=$CP:$(find -L ${ATLAS_WEBAPP_DIR}/atlas/WEB-INF/lib/ -name 'atlas-graphdb-janus*.jar' | tr '\n' ':')
#CP=$CP:$(find -L ${ATLAS_WEBAPP_DIR}/atlas/WEB-INF/lib/ -name 'atlas-intg*.jar' | tr '\n' ':')
CP=$CP:$(find -L ${ATLAS_WEBAPP_DIR}/WEB-INF/lib/ -name 'atlas-graphdb-janus*.jar' | tr '\n' ':')
CP=$CP:$(find -L ${ATLAS_WEBAPP_DIR}/WEB-INF/lib/ -name 'atlas-intg*.jar' | tr '\n' ':')
...
if [ -z "${JAAS_CONF_FILE}" ]; then
  ...
  JAAS_CONF_FILE_PATH=${DIR}/../conf/${JAAS_CONF_FILE}
...
exec $JAVA $JAVA_OPTIONS $MAIN_CLASS -i scripts/startup.groovy "$@"
